...
class Dictionary extends Common
{
    ...
    public function getDictionary(){
        ...
        if($dictionary_id == 'all'){
            $dictionary_id = 0;
            $result = Db::table('dictionary')->field('dictionary_id,parent_id,name,alias,description,valid')->order('order_num')->select();
        }else{
            $where = [
            ...
            $result = Db::table('dictionary')->field('dictionary_id,parent_id,name,alias,description,valid')->where($where)->whereOr($orWhere)->order('order_num')->select();
        }
